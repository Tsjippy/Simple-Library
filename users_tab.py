...
def build_tab():
    ...
    if not constants.window == '' and 'users_main_frame' in constants.window.key_dict:
        constants.window['users_main_frame'].Widget.master.pack_forget()

    users           = [i[1] for i in constants.db.get_db_data(f'SELECT * FROM "main"."Users" ORDER BY last_name', False)]
    if len(users) > 27:
        no_scrollbar    = False
    else:
        no_scrollbar    = True
